...
namespace SSI.Business.SystemManage
{
    public class MenuBLL : RepositoryFactory<T_Menu>
    {
        ...
        public List<T_Menu> GetListByUserId(string userId)
        {
        ...
        public List<T_Menu> GetTreeByUserId(string userId)
        {
            //string sql = string.Format(
            //@"SELECT DISTINCT T1.*
            //  FROM T_MENU T1
            ...
            @"WITH W1 AS (
	            ...
	            FROM
		            T_MENU T1
	            LEFT JOIN T_ACTION T2 ON T2.F_MENU_ID = T1.F_ID
	            LEFT JOIN T_ROLE_ACTION T3 ON T3.F_ACTION_ID = T2.F_ID
	            LEFT JOIN T_USER_ROLE T4 ON T4.F_ROLE_ID = T3.F_ROLE_ID
	            WHERE
		            T1.F_DELETE_MARK = 0
	            AND T2.F_DELETE_MARK = 0
	            AND T2.F_TYPE_MARK = 1
