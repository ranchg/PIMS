...
namespace SSI.Repository
{
    public class Repository<T> : IRepository<T> where T : new()
    {
        ...
        /// <returns></returns>
        public List<T> FindListPage(ref GridParam gp)
        {
            int pageIndex = gp.offset / gp.limit + 1;
            int pageSize = gp.limit;
            int totalRow = 0;
            string querySql = string.Empty;
            List<T> List = DataFactory.Database().FindListPage<T>(gp.sort, gp.order, pageIndex, pageSize, ref totalRow, out querySql);
            gp.total = totalRow;
        ...
        private void AddGridSql2Cahce(string sql)
        {
            DataCahceHelper<T>.AddGridSql(sql);
        }
        #endregion
        ...
        public DataTable FindTable(List<string> queryField)
        {
            return DataFactory.Database().FindTable<T>(queryField);
        }

        public DataTable FindTable(List<string> queryField, string WhereSql, DbParameter[] parameters)
        {
            return DataFactory.Database().FindTable<T>(queryField, WhereSql, parameters);
        }
        /// <summary>
        /// 查询数据列表、返回 DataTable
        /// </summary>
