...
namespace SSI.Utilities
{
    ...
    {
        #region 写文件
        public static bool Write_Dbc(string FileName, string Content)
        {
            bool writeB = false;
            Encoding code = Encoding.GetEncoding("gb2312");
            string htmlfilename = string.Format(HttpContext.Current.Server.MapPath("~/DBCFile/") + FileName + ".dbc");　//保存文件的路径
            if (!System.IO.File.Exists(htmlfilename))
            {
                System.IO.FileStream f = System.IO.File.Create(htmlfilename);
                f.Close();
                f.Dispose();
            }
            string str = Content;
        ...
        #region 读文件
        public static string Read_Dbc(string filename)
        {

            Encoding code = Encoding.GetEncoding("gb2312");
            string temp = string.Format(@"d:\dbcfile\" + filename + ".dbc");
            string str = "";
            if (File.Exists(temp))
            {
